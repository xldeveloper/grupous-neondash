...
export function LeadCard({ lead, onClick, isSelected, onToggleSelect, isSelectMode }: LeadCardProps) {
  const {
    attributes,
    listeners,
    setNodeRef,
  ...
  return (
    <div
      ...
      className={cn(
        ...
        // Dragging state
        isDragging && "opacity-50 rotate-2 scale-105 shadow-xl cursor-grabbing z-50 ring-2 ring-primary",
        // Selected state
      ...
      {lead.tags && lead.tags.length > 0 && (
        <div className="flex flex-wrap gap-1">
            {lead.tags.slice(0, 3).map(tag => (
                <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded-md bg-secondary text-secondary-foreground font-medium border border-secondary-foreground/10">
                    {tag}
                </span>
            ))}
            {lead.tags.length > 3 && (
