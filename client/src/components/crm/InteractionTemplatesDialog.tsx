      ...
      <DialogContent className="sm:max-w-lg h-[80vh] flex flex-col">
        ...
        <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="flex-1 flex flex-col min-h-0">
                                 ...
                                 <div className="flex justify-between items-start">
                                     <div className="flex-1" onClick={() => onSelectTemplate?.(template)}>
                                         <h4 className="font-medium text-sm flex items-center gap-2">
                                             {template.title}
                                             <span className="text-[10px] bg-muted px-1.5 py-0.5 rounded uppercase">{template.type}</span>
                                         </h4>
                                     ...
                                     <div className="flex gap-1 ml-2">
                                         ...
                                         <Button variant="ghost" size="icon" className="h-6 w-6 hover:text-destructive" onClick={() => {
                                             if(confirm("Excluir template?")) deleteMutation.mutate({id: template.id})
                                         }}>
          ...
          <TabsContent value="create" className="flex-1 mt-0">
             <TemplateForm form={form} onSubmit={onSubmit} isPending={createMutation.isPending} onCancel={() => setActiveTab("list")} mode="create" />
          </TabsContent>

          <TabsContent value="edit" className="flex-1 mt-0">
             <TemplateForm form={form} onSubmit={onSubmit} isPending={updateMutation.isPending} onCancel={() => setActiveTab("list")} mode="edit" />
          </TabsContent>
        </Tabs>
      </DialogContent>
    </Dialog>
